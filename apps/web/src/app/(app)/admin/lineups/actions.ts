...
export async function saveLineup(input: { id: string | null; eventId: string | null; name: string; boatType: BoatType; data: Lineup; published: boolean }) {
  const { org, userId } = await requireAdmin();
  const supabase = await createClient();
  const row = { org_id: org.id, event_id: input.eventId, name: input.name, boat_type: input.boatType, data: input.data as unknown as Json, published: input.published, created_by: userId };
  const q = input.id
